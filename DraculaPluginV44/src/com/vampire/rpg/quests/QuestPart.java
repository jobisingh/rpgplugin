package com.vampire.rpg.quests;
...
import com.vampire.rpg.items.RPGItem;
import com.vampire.rpg.utils.VamUtils;
import com.vampire.rpg.warps.WarpCallback;
...
public class QuestPart {
  ...
  //  public Unlock giveUnlock;
    ...
    public boolean interact(QuestVillager qv, Player p, PlayerData pd) {
        if (id == 0) {
            p.sendMessage("");
        ...
        if (giveItems != null && !giveItems.isEmpty()) {
            ...
            if (!VamUtils.hasEmptySpaces(p, giveItems.size())) {
                p.sendMessage("");
