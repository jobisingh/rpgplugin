...
public class ManagerInstances {
    ...
    private static HashMap<Class<? extends AbstractManager>, Pluginc> associatedPlugin = new HashMap<Class<? extends AbstractManager>, Pluginc>();

    protected static final void registerManager(Class<? extends AbstractManager> clazz, AbstractManager instance, Pluginc pl) throws Exception {
    ...
    protected static final void unloadManager(Class<? extends AbstractManager> clazz) {
        if (instances.containsKey(clazz)) {
            ...
            Pluginc pl = associatedPlugin.remove(clazz);
            manager.unload(pl);
    ...
    protected static final void cleanup() {
        for (Entry<Class<? extends AbstractManager>, AbstractManager> e : instances.entrySet()) {
            Pluginc pl = associatedPlugin.get(e.getKey());
