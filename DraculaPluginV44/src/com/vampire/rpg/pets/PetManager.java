...
public class PetManager extends AbstractManager {
    ...
    public PetManager(Pluginc plugin) {
        super(plugin);
    }
    ...
    public void showMenu(Player p, PlayerData pd) {
        ...
        for (PetType pt : PetType.values()) {
            list.add(new MenuItem(row, col, pt == PetType.LIL_ME ? ItemAPI.getPlayerSkull(p.getName()) : pt.item, ChatColor.YELLOW + pt.display, new String[] {
