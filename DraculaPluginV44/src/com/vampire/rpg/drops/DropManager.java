...
public class DropManager extends AbstractManager {
    ...
    public DropManager(Pluginc plugin) {
        super(plugin);
    }
    ...
    public void pickupItemEvent(PlayerPickupItemEvent event) {
        ...
        if (itemMapTime.containsKey(iuuid)) {
            if (System.currentTimeMillis() - itemMapTime.get(iuuid) < 10000) {
                ...
                PlayerData pdo = plugin.getPD(owner);
                PlayerData pd = plugin.getPD(event.getPlayer());
                if (owneruuid == null || owneruuid.equals(event.getPlayer().getUniqueId()) || (pdo != null && pd != null && pd.party != null && pd.party != pdo.party && pd.party.isLootShare())) {
                    itemMapTime.remove(iuuid);
                    itemMapOwner.remove(iuuid);
                    return; // all good
                } else {
                    event.setCancelled(true);
                    if (plugin.getPD(event.getPlayer()) != null && plugin.getPD(event.getPlayer()).getOption(Option.ITEM_PROTECT)) {
    ...
    public void attachLabel(Item item, String name) {
        ArmorStand as = VamTags.makeStand(name, item.getLocation(), true);
...
*/
    ...
    public void onItemDespawn(ItemDespawnEvent event) {
        if (event.getEntity().hasMetadata(VamMetadata.META_NO_PICKUP)) {
            event.setCancelled(true);
            return;
        }
        removeLabel(event.getEntity());
    ...
    public void onItemMerge(ItemMergeEvent event) {
        if (event.getEntity().hasMetadata(VamMetadata.META_NO_PICKUP) || event.getTarget().hasMetadata(VamMetadata.META_NO_PICKUP)) {
            event.setCancelled(true);
            return;
        }
        removeLabel(event.getEntity());
    ...
    public void onItemPickup(PlayerPickupItemEvent event) {
        if (event.getItem().hasMetadata(VamMetadata.META_NO_PICKUP)) {
            event.setCancelled(true);
            return;
        }
