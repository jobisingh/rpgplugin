...
public enum Trinket {
    ...
    RECOVERY("Recovery", PotionType.JUMP, new TrinketStat[] { TrinketStat.HP_MEDIUM, TrinketStat.DEFENSE_SMALL }, new TrinketSpell() {
        ...
        @Override
        public boolean cast(Player p, PlayerData pd) {
    ...
    HAWKEYE("Hawkeye", PotionType.POISON, new TrinketStat[] { TrinketStat.CRIT_CHANCE_LARGE, TrinketStat.DAMAGE_MULTIPLIER_SMALL }, new TrinketSpell() {
        ...
        @Override
        public boolean cast(Player p, final PlayerData pd) {
    ...
    RUNNER("Runner", PotionType.SPEED, new TrinketStat[] { TrinketStat.SPEED_LARGE }, new TrinketSpell() {
        ...
        @Override
        public boolean cast(Player p, final PlayerData pd) {
            p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, VamTicks.seconds(10), 2), false);
            return true;
        }
    ...
    public ArrayList<String> getLore(PlayerData pd) {
        ...
        lore.addAll(VamFormatter.stringToLore(spell.getDescription(), ChatColor.AQUA));
    ...
    public ItemStack createItem(PlayerData pd, ArrayList<String> lore) {
    ...
    public void updateTrinket(PlayerData pd, ItemStack item) {
        if (pd == null)
            return;
        if (!item.hasItemMeta())
            return;
    ...
    public ItemStack getEquipItem(PlayerData pd) {
        return createItem(pd, getLore(pd));
    }

    public ItemStack getDisplayItem(PlayerData pd) {
        ArrayList<String> lore = getLore(pd);
        if (pd.trinket != null && pd.trinket == this)
