...
public class TrinketManager extends AbstractManager {

    public TrinketManager(Pluginc plugin) {
        super(plugin);
    }
            ...
            if (pd != null && pd.isValid()) {
                if (pd.trinket != null) {
                    if (System.currentTimeMillis() > pd.nextTrinketCast) {
                        ...
                        VamParticles.showWithOffsetPositiveY(ParticleEffect.VILLAGER_HAPPY, event.getPlayer().getLocation().add(0, 1, 0), 1.0, 10);
    ...
    public static void showMenu(Player p, final PlayerData pd) {
        p.closeInventory();
