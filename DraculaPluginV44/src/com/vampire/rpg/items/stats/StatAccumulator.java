package com.vampire.rpg.items.stats;
...
public class StatAccumulator {
	 public int level = 1;
	    ...
	    public static void clearStats(PlayerData pd) {
	        pd.damageLow = 0;
	        pd.damageHigh = 0;
	        pd.maxHP = 0;
	    ...
	    public static void finalizeStats(PlayerData pd) {
	    ...
	    public void updgradeStats(double percent){
	    	this.attackDamage*=percent;
	    	this.attackSpeed*=percent;
	    	this.critChance*=percent;
	    	this.critDamage*=percent;
	    	this.damageHigh*=percent;
	    	this.damageLow*=percent;
	    	this.defense*=percent;
	    	this.defenseMultiplier*=percent;
	    	this.hpRegen*=percent;
	    	this.lifeSteal*=percent;
	    	this.spellDamage*=percent;
	    	this.manaRegenRate*=percent;
	    	this.maxHP*=percent;
	    	this.maxHPMultiplier*=percent;
	    	this.rarityFinder*=percent;
	    	this.speed*=percent;

	    }
	    public List<String> lore(double percent) {
	    	ArrayList<String> lore = new ArrayList<String>();
	        boolean space = true;
	        lore.add(StatParser.LEVEL.format(level));
	        if (damageLow > 0 && damageHigh > 0) {
	            lore.add(StatParser.DAMAGE.format(damageLow, damageHigh) +ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(damageLow*percent))+")"+" "+"(+"+String.valueOf((int )(damageHigh*percent))+")");
	        }
	        space = checkSpace(space, lore);
	        if (maxHP > 0) {
	            lore.add(StatParser.HP.format(maxHP)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(maxHP*percent))+")");
	            space = true;
	        }
	        if (maxHPMultiplier > 0) {
	            lore.add(StatParser.HP_MULT.format(maxHPMultiplier)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(maxHPMultiplier*percent))+")");
	            space = true;
	        }
	        if (defense > 0) {
	            lore.add(StatParser.DEFENSE.format(defense)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(defense*percent))+")");
	            space = true;
	        }
	        if (defenseMultiplier > 0) {
	            lore.add(StatParser.DEFENSE_MULT.format(defenseMultiplier)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(defenseMultiplier*percent))+")");
	            space = true;
	        }
	        space = checkSpace(space, lore);
	        if (critChance > 0) {
	            lore.add(StatParser.CRIT_CHANCE.format(critChance)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(critChance*percent))+")");
	            space = true;
	        }
	        if (critDamage > 0) {
	            lore.add(StatParser.CRIT_DAMAGE.format(critDamage)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(critDamage*percent))+")");
	            space = true;
	        }
	        space = checkSpace(space, lore);
	        if (rarityFinder > 0) {
	            lore.add(StatParser.RARITY_FINDER.format(rarityFinder)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(rarityFinder*percent))+")");
	            space = true;
	        }
	        if (manaRegenRate > 0) {
	            lore.add(StatParser.MANA_REGEN.format(manaRegenRate)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(manaRegenRate*percent))+")");
	            space = true;
	        }
	        if (speed > 0) {
	            lore.add(StatParser.SPEED.format(speed)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(speed*percent))+")");
	            space = true;
	        }
	        if (spellDamage > 0) {
	            lore.add(StatParser.SPELL_DAMAGE.format(spellDamage)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(spellDamage*percent))+")");
	            space = true;
	        }
	        if(lifeSteal>0){
	        	lore.add(StatParser.LIFE_STEAL.format(lifeSteal)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(lifeSteal*percent))+")");
	        }
	        if (attackDamage > 0) {
	            lore.add(StatParser.ATTACK_DAMAGE.format(attackDamage)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(attackDamage*percent))+")");
	            space = true;
	        }
	        if (hpRegen > 0) {
	            lore.add(StatParser.HP_REGEN.format(hpRegen)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(hpRegen*percent))+")");
	            space = true;
	        }
	        if (attackSpeed > 0) {
	            lore.add(StatParser.ATTACK_SPEED.format(attackSpeed)+ChatColor.WHITE+" "+ChatColor.BOLD+"(+"+String.valueOf((int )(attackSpeed*percent))+")");
	            space = true;
	        }
	        space = checkSpace(space, lore);
	    ...
	    public List<String> lore() {
	        ...
	        if(lifeSteal>0){
	        	lore.add(StatParser.LIFE_STEAL.format(lifeSteal));
	        }
	        if (attackDamage > 0) {
