...
public enum StatParser {
    ...
    LIFE_STEAL(ChatColor.DARK_BLUE+"Life Steal "+ChatColor.WHITE+"+%d%%", new EquipStatParser(){
		...
		try{
			if(!s.contains("Life Steal"))
				return;
			int val = Integer.parseInt(s.substring("Life Steal +".length(),s.lastIndexOf('%')));
			sa.setLifeSteal(val);
			}catch(Exception e){
				return;
			}
    ...
    public static boolean isStatLine(String s){
    	   if (!s.contains("Level") && (!s.contains("HP") || s.contains("%")) && !(s.contains("HP") && s.contains("%")) && (!s.contains("Defense") || s.contains("%")) && !(s.contains("Defense") && s.contains("%")) && (!(s.contains("Damage") && s.contains("-"))) && !s.contains("Crit Chance") && !s.contains("Crit Damage") && !s.contains("Treasure Hunter") && !s.contains("Mana Regen Rate") && !s.contains("Speed") && !s.contains("Spell Damage") && !s.contains("Attack Damage") && !s.contains("HP Regen") && !s.contains("Attack Speed")){
    		   return false;
    	   }
    	   return true;
