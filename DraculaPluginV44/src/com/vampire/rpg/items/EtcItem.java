...
import com.vampire.rpg.utils.VamFormatter;
...
public class EtcItem extends RPGItem {
    ...
    public ItemStack generate() {
        ...
        if (description.length() > 0)
            lore.addAll(VamFormatter.stringToLore(description, ChatColor.GRAY));
        im.setLore(lore);
    ...
    public static void postInitialize() {
        ...
        ItemManager.registerItemRunnable("old_player", new ItemRunnable() {
            @Override
            public void run(Event e, Player p, PlayerData pd) {
                ...
                for (Location loc : locs)
                    VamParticles.spawnRandomFirework(loc);
        ...
        ItemManager.registerItemRunnable("july_2014_voter", new ItemRunnable() {
            @Override
            public void run(Event event, final Player p, PlayerData pd) {
                ...
                VamParticles.show(ParticleEffect.FIREWORKS_SPARK, loc);
                VamParticles.show(ParticleEffect.NOTE, loc);
                VamParticles.show(ParticleEffect.SNOWBALL, loc);
                VamParticles.show(ParticleEffect.HEART, loc);
        ...
        ItemManager.registerItemRunnable("hp_potion_1", new ItemRunnable() {
            @Override
            public void run(Event event, Player p, PlayerData pd) {
        ...
        ItemManager.registerItemRunnable("melon", new ItemRunnable() {
            ...
            public void run(Event event, Player p, PlayerData pd) {
                ...
                pd.heal(80);
                VamSound.playSound(p, Sound.ENTITY_GENERIC_EAT);
            }
        });
        ItemManager.registerItemRunnable("me", new ItemRunnable() {
			
			@Override
			public void run(Event event, Player p, PlayerData pd) {
				p.sendMessage(ChatColor.BLACK+"WOW");
        ...
        ItemManager.registerItemRunnable("maru_milk", new ItemRunnable() {
            ...
            public void run(Event event, Player p, PlayerData pd) {
                ...
                p.addPotionEffect(new PotionEffect(PotionEffectType.CONFUSION, VamTicks.seconds(30), 1), false);
                p.addPotionEffect(new PotionEffect(PotionEffectType.SLOW, VamTicks.seconds(30), 2), false);
                p.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_DIGGING, VamTicks.seconds(30), 0), false);
            }
        });
        ...
        ItemManager.registerItemRunnable("tortuga_ale", new ItemRunnable() {
            ...
            public void run(Event event, Player p, PlayerData pd) {
                takeOneItemInstant(p);
                pd.heal(pd.getCurrentMaxHP());
                p.addPotionEffect(new PotionEffect(PotionEffectType.CONFUSION, VamTicks.seconds(15), 1), false);
                p.addPotionEffect(new PotionEffect(PotionEffectType.SLOW, VamTicks.seconds(15), 2), false);
                p.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_DIGGING, VamTicks.seconds(15), 0), false);
                pd.sendMessage(ChatColor.GRAY + "> " + "The ale tastes terrible, and now you feel a little tipsy.");
            }
        });
        ItemManager.registerItemRunnable("compass", new ItemRunnable() {			
			@Override
			public void run(Event event, Player p, PlayerData pd) {
        ...
        ItemManager.registerItemRunnable("rotten_apple", new ItemRunnable() {
            ...
            public void run(Event event, Player p, PlayerData pd) {
                ...
                pd.heal(300);
                if (Math.random() < 0.5) {
                    p.addPotionEffect(new PotionEffect(PotionEffectType.CONFUSION, VamTicks.seconds(10), 1), false);
                    p.addPotionEffect(new PotionEffect(PotionEffectType.SLOW, VamTicks.seconds(10), 2), false);
                }
            }
        });
        ...
        ItemManager.registerItemRunnable("mana_pot_1", new ItemRunnable() {
            ...
            public void run(Event event, Player p, PlayerData pd) {
                ...
                if (pd.mana < PlayerData.MAX_MANA) {
                    takeOneItemInstant(p);
                    pd.recoverMana(2);
                }
            }
    ...
    private static boolean justAte(PlayerData pd, String id) {
    ...
    public static void healWithPotion(int amount, String name, Event event, Player p) {
        ...
        VamSound.playSound(p, Sound.ENTITY_GENERIC_DRINK);
    }
