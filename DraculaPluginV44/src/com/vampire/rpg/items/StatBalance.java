...
public class StatBalance {
    ...
    public static int getDurabillityBonus(int durb){
    	int temp;
    	switch(durb){
    	case 0:
    		temp= -4;
    ...
    public static int getManaRegenRate(int level) {
        ...
        int range = VamMath.randInt(-10, 10);
        core += range;
        if (core < 1)
            core = 1;
        return core;
   ...
   /* public static int getLifeSteal(int level){
    ...
    public static int getSpeed(int level) {
        return VamMath.randInt(5, 15);
    }
    public static int getUpgradeCosts(int itemLevel,int rarityInt,int numberOfUpgrades){
    	...
    	int ad=(int) Math.floor(itemLevel*40*rarityMult);
    	int k=0;
    	while(k<numberOfUpgrades){
    		ad*=1.3;
    		k++;
    	}
    	return ad;
