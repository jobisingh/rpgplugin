        ...
        for (int k = 0; k < numberOfArrows; k++) {
            VamScheduler.schedule(Spell.plugin, new Runnable() {
                public void run() {
                    ...
                    p.setMetadata(VamMetadata.META_DAMAGE, new FixedMetadataValue(Spell.plugin, fDamage));
                    p.setMetadata(VamMetadata.META_KNOCKBACK, new FixedMetadataValue(Spell.plugin, 0));
                }
            }, k * 3);
        }
