...
//    public BossAIRunnable bossAI = null;
  ...
  //  public Dungeon dungeon = null;
    ...
    public void startTasks() {
        ...
        VamScheduler.schedule(Pluginc.getInstance(), ai, 5);
        spellTicker.start();
    }
    ...
    public void die(boolean drop) {
        ...
        VamTags.makeFloatingText(ChatColor.GOLD + "+" + mobType.exp * (elite ? 25 : 1) + " EXP", entity.getLocation(), 1.3, 1.0, 1.5, 1);
        if (mount != null) {
            try {
                mount.eject();
                entity.leaveVehicle();
        ...
        entity.damage(1000000000);
                VamParticles.showWithOffset(ParticleEffect.REDSTONE, entity.getLocation().add(0, 1, 0), 1.3, 10);
                VamParticles.showWithDataAndOffset(ParticleEffect.BLOCK_CRACK, entity.getLocation().add(0, 1, 0), new BlockData(Material.LAVA, (byte) 0), 10, 1.2, 0.4, 1.2);
                VamParticles.showWithData(ParticleEffect.BLOCK_CRACK, entity.getLocation().add(0, 1, 0), new BlockData(Material.LAVA, (byte) 0), 10);
    ...
    //    }
        ...
        long mostDealt = 0;
        UUID priority = null;
        PlayerData topDamage = null;
        for (Entry<UUID, Long> entry : damageDealt.entrySet()) {
            Player p = plugin.getServer().getPlayer(entry.getKey());
            if (p == null)
                continue;
            if (VamMath.flatDistance(p.getLocation(), deathLoc) < 50) {
        ...
        if (drop || this.hp < this.mobType.maxHP * (elite ? 10 : 1)) {
            ...
            for (MobDrop md : mobType.drops) {
                if (md.roll()) {
                    int amount = VamMath.randInt(md.minAmount, md.maxAmount);
            ...
            if (!attributes.contains(MobAttribute.NODROP)) {
                if (Math.random() < MobBalance.getDropRate(mobType.level)) {
                    ...
                    int lv = VamMath.randInt(low, high);
                    if (lv < 1)
                        lv = 1;
                    ItemStack item;
   ...
   //     this.bossAI = null;
    ...
    public int getDamage() {
        return VamMath.randInt(mobType.damageLow, mobType.damageHigh);
    }

    public boolean attack(PlayerData pd) {
        return attack(pd, 0.5, -1, false);
    }

    public boolean attack(PlayerData pd, int rpgDamage) {
        return attack(pd, 0.5, rpgDamage, false);
    }

    public boolean attack(PlayerData pd, int rpgDamage, boolean projectile) {
        return attack(pd, 0.5, rpgDamage, projectile);
    }

    public boolean attack(PlayerData pd, double knockback, int rpgDamage, boolean projectile) {
    ...
    public boolean damage(int damageAmount, Entity damager, DamageType damageType, boolean crit) {
        ...
        if (damager != null && damager instanceof Player) {
            if (!((Player) damager).isOnline() || Pluginc.getPD((Player) damager) == null)
                return false;
            Player p= (Player)damager;
         //   p.sendMessage("still testing");
        ...
        if (hp < 1) {
           ...
           VamParticles.showWithData(ParticleEffect.BLOCK_CRACK, entity.getLocation().add(0, 1.5, 0), new BlockData(Material.REDSTONE_BLOCK, (byte) 0), 10);
                       VamScheduler.schedule(plugin, () -> {
            entity.playEffect(EntityEffect.HURT);
                       });
            if (mount != null)
    ...
    private void displayCurrentHP() {
        ...
        VamScheduler.schedule(Pluginc.getInstance(), lastScheduledRevertHP = new Runnable() {
            public void run() {
    ...
    public void regen(double value) {
        ...
        VamParticles.show(ParticleEffect.HEART, entity.getLocation());
        displayCurrentHP();
    }
    ...
    public void playCrit() {
        if (entity == null || !entity.isValid())
            return;
       VamParticles.showWithOffset(ParticleEffect.CRIT, entity.getEyeLocation(), 1, 10);
    }
    ...
    public void shout(String s, int radius) {
        for (Entity e : VamMath.getNearbyEntities(entity.getLocation(), radius)) {
