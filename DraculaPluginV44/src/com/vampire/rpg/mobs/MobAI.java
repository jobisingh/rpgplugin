...
public class MobAI implements Runnable {
    ...
    private static final double TICKS_TO_CHANGE_TARGET = VamTicks.seconds(10d);
    ...
    public void run() {
        ...
        double distanceAwayFromOriginal = VamMath.flatDistance(currentLoc, originalLoc);
        ...
        if (aggressive) {
            if (!forceReturn) {
                ...
                if (target == null || MobData.plugin.getPD(target) == null || MobData.plugin.getPD(target).isStealthed() ||!MobData.plugin.getPD(target).isPVE() || VamMath.flatDistance(target.getLocation(), originalLoc) > MAX_DISTANCE_FROM_ORIGINAL || target.isDead() || !target.isValid() || !target.isOnline()) {
                    target = null;
                    Player closest = null;
                    ...
                    if (counter % 4 == 1) {
                        ...
                        for (Player p : Pluginc.getInstance().getServer().getOnlinePlayers()) {
                            double dist = VamMath.flatDistance(p.getLocation(), currentLoc);
            ...
            if (target != null && counter % 12 == 0) { // 4 ticks per second
                PlayerData pd = MobManager.plugin.getPD(target);
                if (pd != null && pd.isValid()) {
                    ...
                    if (pd.safespotCounter > SAFESPOT_THRESHOLD) {
                        ...
                        if (!ignore.contains(ignoreUUID)) {
                            ...
                            VamScheduler.schedule(MobManager.plugin, () -> {
                                if (ignore.contains(ignoreUUID)) {
            ...
            if (target != null && !forceReturn) {
                ...
                if (ranged) {
                    ...
                    try {
                        ...
                        if (VamMath.flatDistance(currentLoc, dest) < rangeDistance) {
                            dest = currentLoc;
                            attemptAttack = true;
                        }
                        navigate();
                    ...
                    try {
                        ...
                        if (target != null && Math.abs(targetLoc.getX() - currentLoc.getX()) < xdiff && Math.abs(targetLoc.getZ() - currentLoc.getZ()) < xdiff && Math.abs(targetLoc.getY() - currentLoc.getY()) < ydiff) {
                            PlayerData pd = MobData.plugin.getPD(target);
                            if (pd != null && !md.dead) {
                                if (md.attack(pd)) {
                                    if (entity instanceof IronGolem)
                                        nmsEntity.world.broadcastEntityEffect(nmsEntity, (byte) 4); // Iron Golem attack animation
                ...
                if (counter % 4 == 0 && md.hp < md.mobType.maxHP && lastAttackTickCounter > VamTicks.seconds(5)) {
                    md.regen(md.mobType.maxHP * NO_TARGET_REGEN_HP_PERCENTAGE);
                    target = null;
                    dest = originalLoc.clone();
                    if (nmsEntity instanceof CustomMagmaCube) {
                        VamParticles.showWithOffsetPositiveY(ParticleEffect.FLAME, entity.getLocation(), 1.5, 75);
                        entity.teleport(dest);
                        target = null;
                    } else if (entity instanceof Slime) {
                       VamParticles.showWithOffsetPositiveY(ParticleEffect.SLIME, entity.getLocation(), 1.5, 75);
                        entity.teleport(dest);
                        target = null;
                    }
                    navigate();
        ...
        VamScheduler.schedule(Pluginc.getInstance(), this, 3 + (int) (Math.random() * 5));
    }
    ...
    private void navigate() {
        ...
        if (forcedDest != null) {
            ...
            if (System.currentTimeMillis() - lastDirChange > VamMath.randInt(5000, 10000) || lastDir == null) {
                Vector v = Vector.getRandom();
                v.setX(v.getX() - 0.5f);
                v.setZ(v.getZ() - 0.5f); // center around origin
                lastDir = v.setY(0).normalize().multiply(VamMath.randInt(3, 5));
            ...
            if (System.currentTimeMillis() - lastDirChange > VamMath.randInt(5000, 10000) || lastDir == null) {
                ...
                v.setX(v.getX() - 0.5f);
                v.setZ(v.getZ() - 0.5f); // center around origin
                lastDir = v.setY(0).normalize().multiply(VamMath.randInt(1, 3));
                lastDirChange = System.currentTimeMillis();
            }
            dest.add(lastDir);
        }
        AttributeInstance ati = ((EntityLiving) (((CraftLivingEntity) entity).getHandle())).getAttributeInstance(GenericAttributes.FOLLOW_RANGE);
        double old = ati.getValue();
        double diff = VamMath.flatDistance(dest, entity.getLocation());
        if (old < diff) {
