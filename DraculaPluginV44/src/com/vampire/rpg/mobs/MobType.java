...
public class MobType {
    ...
    public MobData spawn(Location loc, int leash, String name) {
        ...
        // spawn main mob
        LivingEntity le = VamEntities.createLivingEntity(entityClass, loc);
   ...
   //     }
        ...
        if (le instanceof Ocelot) {
            ((Ocelot) le).setCatType(VamMath.randObject(Ocelot.Type.values()));
        }
        ...
        if (attributes.contains(MobAttribute.HORSE1)) {
            Horse horseMount = (Horse) VamEntities.createLivingEntity(CustomHorse.class, md.entity.getLocation());
