...
public class SlimeKingBurstSpell extends MobSpell {
    ...
    public void castSpell(final LivingEntity caster, final MobData md, Player target) {
        md.shout((String) VamMath.randObject(PHRASES), 20);
        ...
        for (Vector v : getVectors(md.entity)) {
            ArrayList<Location> locs = VamMath.calculateVectorPath(start.clone(), v, 15, 4);
            int count = 0;
            for (int k = 0; k < locs.size(); k++) {
                final Location loc = locs.get(k);
                VamScheduler.schedule(Spell.plugin, new Runnable() {
                    public void run() {
                        VamParticles.showWithOffset(ParticleEffect.SLIME, loc, 0.2, 1);
