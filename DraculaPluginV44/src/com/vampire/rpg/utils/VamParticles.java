...
public class VamParticles {
    ...
    public static void show(ParticleEffect effect,Location loc, int amount,int radius){
        effect.display(0, 0, 0, 0, amount, loc, radius);
    }
    ...
    public static void showParticles(Particle p,Location loc,Option o){
    	for(Entity e:VamMath.getNearbyEntities(loc, 60)){
    		if(!(e instanceof Player))
    			continue;
    		Player pl=(Player)e;
    		PlayerData pd=Pluginc.getPD(p);
    		if(pd.getOption(o))
    		pl.spawnParticle(p, loc.getX(),loc.getY(),loc.getZ(),100);
    		PacketContainer wrap= new PacketContainer(PacketType.Play.Server.ENTITY_EFFECT);
    		WrapperPlayServerEntityEffect wr= new WrapperPlayServerEntityEffect(wrap);
    	}
    }
    /**
    ...
    public static Firework spawnRandomFirework(Location loc) {
        ...
        Color c1 = Color.fromRGB(VamMath.randInt(0, 255), VamMath.randInt(0, 255), VamMath.randInt(0, 255));
        Color c2 = Color.fromRGB(VamMath.randInt(0, 255), VamMath.randInt(0, 255), VamMath.randInt(0, 255));
        ...
        fwm.setPower(VamMath.randInt(1, 2));
