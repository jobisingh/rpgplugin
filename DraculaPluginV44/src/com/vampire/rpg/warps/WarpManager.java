...
public class WarpManager extends AbstractManager {
                    ...
                    try {
                        ...
                        if (lastLoc == null) {
                            ...
                            if (Math.abs(curr.getX() - lastLoc.getX()) > THRESHOLD || Math.abs(curr.getY() - lastLoc.getY()) > THRESHOLD || Math.abs(curr.getZ() - lastLoc.getZ()) > THRESHOLD) {
                                ...
                                VamMessages.sendTitle(p, ChatColor.RED + "Warp cancelled", "", 2, 30, 5);
                                endWarp(false);
                                return;
                            }
                        }
                        ...
                        if (count == 0)
                            VamMessages.sendTitle(p, "Warping now!", ChatColor.GREEN + "Move to cancel the warp", 2, 16, 2);
                        else
                            VamMessages.sendTitle(p, "Warping in " + count, ChatColor.GREEN + "Move to cancel the warp", 2, 16, 2);
                        count--;
                        if (count < 0) {
                            VamParticles.showWithOffset(ParticleEffect.CLOUD, p.getLocation().add(0, 1, 0), 1.5, 30);
                            p.teleport(dest);
                            VamParticles.showWithOffset(ParticleEffect.CLOUD, p.getLocation().add(0, 1, 0), 1.5, 30);
                            endWarp(true);
                        } else {
    ...
    public WarpManager(Pluginc plugin) {
        super(plugin);
    }
