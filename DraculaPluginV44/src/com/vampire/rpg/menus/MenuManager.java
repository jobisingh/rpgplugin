...
public class MenuManager extends AbstractManager {
    ...
    public MenuManager(Pluginc plugin) {
        super(plugin);
    }
    ...
    //    event.getWhoClicked().sendMessage("fuck you menumanager");
        if (invClickables.containsKey(uuid)) {
    ...
    public void onInventoryClose(InventoryCloseEvent event) {
    	//  VamMessages.announce("TEST MENUS");
    ...
    public static Inventory modifyMenu(Player p, Inventory inventory, Object[][] info) {
        for (Object[] data : info) {
            ...
            if (data[2] == null) {
                ...
                item = null;
                VamMessages.announce(ChatColor.RED + "Error 100. Please report to Edasaki or Misaka!");
                return inventory;
            }
            ItemMeta im = item.getItemMeta();
            ...
            if (data[4] instanceof Object[]) {
                ...
                for (int k = 0; k < desc.length; k += 2) {
                    lore.addAll(VamFormatter.stringToLore(desc[k + 1].toString(), desc[k] != null ? desc[k].toString() : ChatColor.GRAY));
                }
                im.setLore(lore);
